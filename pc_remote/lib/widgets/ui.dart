...
/// Lumen Desk mark: deep-navy tile, champagne-gold brightness dial.
/// Drawn in code so it matches on Android, Windows and in-app.
...
class _LogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Tile: deep navy gradient, rounded.
    final tile = Paint()
      ..shader = const LinearGradient(
        colors: [Color(0xFF0C1626), Color(0xFF172942)],
        begin: Alignment.topCenter,
        end: Alignment.bottomCenter,
      ).createShader(rect);
    canvas.drawRRect(
        RRect.fromRectAndRadius(rect, Radius.circular(r * 0.22)), tile);
    const gold = Color(0xFFD9B36A);
    final cx = r / 2, cy = r * 0.52;
    final radius = r * 0.26;
    // Dial arc with a gap at the bottom.
    canvas.drawArc(
    ...
    // End dot at the gap's right edge (45 deg).
    const a = math.pi / 4;
    canvas.drawCircle(
    ...
    // Core dot.
    canvas.drawCircle(
        Offset(cx, cy), r * 0.022, Paint()..color = gold);
  }
