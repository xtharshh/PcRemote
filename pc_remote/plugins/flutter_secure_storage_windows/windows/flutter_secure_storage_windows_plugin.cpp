...
#include <windows.h>
#include <wincred.h>
// NOTE (LumiLink): atlstr.h needs the VS ATL component, which Build Tools
// installs don't include by default. The plugin only used ATL for CA2W/CW2A
// string conversion, replaced below with plain Win32 converters.
...
namespace
{
  ...
  // Minimal UTF-8 <-> UTF-16 converters (stand-ins for ATL's CA2W/CW2A).
  ...
  inline std::string WideToUtf8(const wchar_t* ws) {
    if (ws == nullptr || *ws == L'\0') return std::string();
